...
public interface HelpRecordRepository extends JpaRepository<HelpRecord, Long> {
    ...
    @Query(nativeQuery = true, value = "SELECT t1.* FROM HelpRecord t1 LEFT JOIN Session t2 ON sessionID WHERE t1.counsellorID = ?1 AND t2.startTimestamp >= ?2 AND t2.startTimestamp <= ?3")
    List<HelpRecord> findByCounsellorIDAndStartTimeAndEndTime(Long counsellorID, Long startTimestamp, Long endTimestamp);

    @Query(nativeQuery = true, value = "SELECT t1.* FROM HelpRecord t1 LEFT JOIN Session t2 ON sessionID WHERE t1.supervisorID = ?1 AND t2.startTimestamp >= ?2 AND t2.startTimestamp <= ?3")
