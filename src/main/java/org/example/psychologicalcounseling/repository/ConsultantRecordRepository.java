...
public interface ConsultantRecordRepository extends JpaRepository<ConsultantRecord, Long> {
    @Query(nativeQuery = true, value = "select t1.recordID, t1.counsellorID, t1.userID, t1.sessionID, t1.userRating, t1.appraisal, t1.counsellorAppraisal " +
            "from ConsultantRecord t1 LEFT JOIN Session t2 ON t1.sessionID = t2.sessionID where t1.userID = ?1 and t1.sessionID = t2.sessionID and t2.startTimestamp >= ?2 and t2.startTimestamp <= ?3")
    List<ConsultantRecord> findByUserIDAndTimestampBetween(Long userID, Long startTimestamp, Long endTimestamp);

    @Query(nativeQuery = true, value = "select t1.recordID, t1.counsellorID, t1.userID, t1.sessionID, t1.userRating, t1.appraisal, t1.counsellorAppraisal " +
            "from ConsultantRecord t1 LEFT JOIN Session t2 ON t1.sessionID = t2.sessionID where t1.counsellorID = ?1 and t1.sessionID = t2.sessionID and t2.startTimestamp >= ?2 and t2.startTimestamp <= ?3")
    ...
    @Query(nativeQuery = true, value = "SELECT COUNT(*) FROM ConsultantRecord WHERE counsellorID = ?1 AND userID = ?2 AND sessionID = ?3")
    Long existsByAttributes(Long counsellorID, Long userID, Long sessionID);
}
