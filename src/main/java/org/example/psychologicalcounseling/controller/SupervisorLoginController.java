...
public class SupervisorLoginController {
    ...
    public ResponseEntity<?> login(@RequestBody LoginRequestDto loginRequest) {
        ...
        if(password!=null && !password.isEmpty()){
            if(supervisorLoginService.checkEmail(email)){
                //登录验证
                if (supervisorLoginService.checkPassword(email, password)) {
                    //给正确的用户生成token
                    //登录成功
            ...
            if(verificationCode==null || verificationCode.isEmpty()){
                //发送验证码
                if(supervisorLoginService.SendVerificationCode(email, VerificationCodeBuilder.generateVerificationCode())){
                    response.setCode(600);
                ...
                //核对验证码
                if(supervisorLoginService.checkVerificationCode(email, verificationCode)){
                    if(!supervisorLoginService.checkEmail(email)){
                        UserDto userDto = new UserDto();
                        userDto.setEmail(email);
                        userDto.setPassword(OriginalPasswordBuilder.generatePassword());
                        userDto.setNickname("User"+email);
                        supervisorLoginService.addUser(userDto);
