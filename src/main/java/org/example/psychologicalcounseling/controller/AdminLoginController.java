...
import org.example.psychologicalcounseling.module.user.login.AdminLoginService;
...
public class AdminLoginController {
    ...
    public ResponseEntity<?> login(@RequestBody LoginRequestDto loginRequest) {
        ...
        if(password!=null && !password.isEmpty()){
            if(adminLoginService.checkEmail(email)){
                //登录验证
                if (adminLoginService.checkPassword(email, password)) {
                    //给正确的用户生成token
                    //登录成功
            ...
            if(verificationCode==null || verificationCode.isEmpty()){
                //发送验证码
                if(adminLoginService.SendVerificationCode(email, VerificationCodeBuilder.generateVerificationCode())){
                    response.setCode(600);
                ...
                //核对验证码
                if(adminLoginService.checkVerificationCode(email, verificationCode)){
                    if(!adminLoginService.checkEmail(email)){
